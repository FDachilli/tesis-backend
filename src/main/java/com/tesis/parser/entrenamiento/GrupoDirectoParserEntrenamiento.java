...
public class GrupoDirectoParserEntrenamiento extends ParserEntrenamiento{
        ...
        try {
            ...
            for (ConversationStateRoot conversationStateRoot : conversationStateRoots) {
                if (!listIdConversacionesIgnoradas.contains(conversationStateRoot.getConversationId().getId()) && conversationStateRoot.getConversationState().getConversation().getName()!=null){
                    //SOLO AGREGAMOS LAS CONVERSACIONES COMPLETAS Y LAS QUE TIENEN NOMBRE (PARA EVITAR LAS CONVERSACIONES INDIVIDUALES CON P.E)
                    Boolean first = true;
                    Date dateFirst = null;
                    String fileContent;
    ...
    public String agregarAtributos(String pathfile, List<Atributos> lista_atributos) {
        ...
        String resultFilePath = Constants.DIRECTO_GRUPOS_LABELED_FOLDER + String.valueOf(System.currentTimeMillis()) + "-roles" + Constants.ARFF_FILE;
